...
def check_translations(input_file):
    ...
    if all_correct:
        print(f"✅ Все переводы корректны (размер, форматные строки и переносы)! ⚠️  Пропущено {skip}")
    else:
        print("❌ Обнаружены ошибки в переводах (размер, форматные строки или переносы)")
        sys.exit(1)
