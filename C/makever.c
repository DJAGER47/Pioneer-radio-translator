...
typedef struct header_st {
  ...
  int zero1;
  int a2c;         // 2c
  int nb0_length;  // payload file lehght
  int nb0_crc;     // payload file CRC32?
  int version1;    // same version
  int volume1;     // 1
  int zero2;
  int nfiles;    // 1
  int dirdepth;  // 1
...
int main(void) {
  ...
  memset(&h_prg, 0, sizeof(h_prg));
  memset(&h, 0, sizeof(h));

  char *dir     = "PLATFORM";
  char *datfile = "PS140PLT.PRG";

  char *nb0file = "output.nb0";
  char *pltname = "PS140PLT.PRG";
  char *pltdate = "2021/04/06_10:04:53+540";
  // "2015/11/21_11:16:12+540";

  int ffs[16] = {0xFFFFFFFF,
                 0xFFFFFFFF,
                 0xFFFFFFFF,
                 0xFFFFFFFF,
                 0xFFFFFFFF,
  ...
  h_prg.magic = 0xa55a5aa5;  // PRG header creation
  fin1        = fopen(nb0file, "rb");
  fseek(fin1, 0L, SEEK_END);
  h_prg.file_length = ftell(fin1);
  unsigned char *p1 = malloc(h_prg.file_length);
  rewind(fin1);
  fread(p1, 1, h_prg.file_length, fin1);
  h_prg.file_crc = checksum32(p1, h_prg.file_length);  // payload checksum
  h_prg.version  = 0x07010000;                         // 0x01003000;
  h_prg.volume   = 2;
  strcpy(h_prg.filename, pltname);
  strcpy(h_prg.filedate, pltdate);
  memcpy(h_prg.ffs0, ffs, sizeof(ffs) - 4);
  h_prg.magic1          = 0xa55a5aa5;
  h_prg.file_length_prg = h_prg.file_length;
  h_prg.file_crc1       = h_prg.file_crc;
  h_prg.magic2[0]       = 0x88471000;
  h_prg.magic2[1]       = 0x88471004;
  h_prg.magic2[2]       = 0x10000006;
  memcpy(h_prg.ffs1, ffs, sizeof(h_prg.ffs1));
  memcpy(h_prg.ffs2, ffs, sizeof(h_prg.ffs2));
  memcpy(h_prg.ffs3, ffs, sizeof(h_prg.ffs3));
  ...
  fseek(fout1, 0L, SEEK_END);
  fwrite(p1, 1, h_prg.file_length, fout1);
  fclose(fout1);
  fclose(fin1);

  h.magic      = 0xa55a5aa5;  // VER file for PLATFORM creation
  h.len        = sizeof(h);
  ...
  h.magic1   = 0xa55a5aa5;
  h.ver_crc  = crc32b((unsigned char *)&h, h.len - 4);
  fout       = fopen("PS140PLT.VER", "wb");
