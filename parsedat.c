...
int main(int argc, char *argv[])  // Parse Japanese AVIC-RZ09 (and similar) data file initDB.dat and write strings with address, lenght, original
                                  // string (UCS-2 LE encoding)
{
  ...
  fwrite(&fffe,
         2,
         1,
         fout1);  // BOM for UCS-2 LE - use Notepad++ (or other UCS-2 LE compatible editor, Chrome is OK for view strings) to see/edit results
  ...
  for (int i = 0; i < (BLOCK_LEN + 2) / 2; i++) {
    ...
    if (p[0] == 0x00 && p[1] == 0x00) {
      if (index > 0) {
        if (index > 8 && str[index - 8] == 0x2E && str[index - 6] == 0x67 && str[index - 4] == 0x69 &&
            str[index - 2] == 0x66) {  // Skip links to images, .gif
          index = 0;
          continue;
        }
        if (index > 8 && str[index - 8] == 0x2E && str[index - 6] == 0x62 && str[index - 4] == 0x6D &&
            str[index - 2] == 0x70) {  // Skip links to images, .bmp
          index = 0;
          continue;
        }
        if (index > 8 && str[index - 8] == 0x2E && str[index - 6] == 0x70 && str[index - 4] == 0x6E &&
            str[index - 2] == 0x67) {  // Skip links to images, .png
          index = 0;
          continue;
        }
        if (index > 8 && str[index - 8] == 0x2E && str[index - 6] == 0x6A && str[index - 4] == 0x70 &&
            str[index - 2] == 0x67) {  // Skip links to images, .jpg
          index = 0;
          continue;
        }
